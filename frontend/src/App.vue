<template>
  <div class="phone-frame">
    <main class="phone-screen">
      ...
      <button class="audio-toggle" :aria-label="isMuted ? 'Unmute background music' : 'Mute background music'"
        @click="toggleAudio">
        <!-- Unmuted (volume) icon -->
        <svg v-if="!isMuted" xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none"
      ...
      <!-- Background music -->
      <audio ref="audioRef" :muted="isMuted" loop preload="auto" playsinline>
      ...
      <!-- Only show bottom nav if not on login/signup -->
      <BottomNavBar v-if="showBottomNav" class="bottom-nav" />
    </main>
  </div>
</template>
...
// Hide navbar for login & signup routes
